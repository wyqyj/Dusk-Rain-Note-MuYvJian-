...
export const useTimerStore = create<TimerStore>((set, get) => ({
  ...
  startTimer: (taskId, noteId, taskText, noteTitle) => {
    ...
    if (state.activeSession) {
      ...
      if (finalMs > 0) {
        ...
        void getBridge().timers.saveTimerRecord(record);
      }
      void getBridge().timers.saveActiveSession(null);
    }
    ...
    void getBridge().timers.saveActiveSession(session);
  ...
  pauseTimer: () => {
    ...
    void getBridge().timers.saveActiveSession(updated);
  },
  ...
  stopTimer: async () => {
    ...
    if (finalMs > 0) {
      ...
      // 等待保存完成
      try {
        await getBridge().timers.saveTimerRecord(record);
        await getBridge().timers.saveActiveSession(null);
      } catch (e) {
  ...
  loadFromDisk: async () => {
    try {
      const session = await getBridge().timers.loadActiveSession();
