...
import {
  EV_QUICK_NOTE_TOGGLE, EV_QUICK_NOTE_CLOSE,
  EV_TODAY_PLAN_TOGGLE, EV_TODAY_PLAN_CLOSE,
  EV_STATS_TOGGLE, EV_STATS_CLOSE, EV_PANEL_BACK,
} from '../adapters/ui-events';
import { useNoteStore, registerReloadListener } from './store/noteStore';
import { useAttachmentStore } from './store/attachmentStore';
import { useUIStore } from './store/uiStore';
import { useSettingsStore } from './store/settingsStore';
import { generateId } from '../core/utils/markdown';
import { getBridge } from '../platform/container';
import { isNativeMobile } from './utils/platform';

/** 手机端画布占位卡：画布仅在桌面版编辑；有章节大纲时只读展示。 */
type CanvasFallbackNote = { canvasOutline?: Array<{ itemId: string; label: string }> };
const MobileCanvasFallback: React.FC<{ note?: CanvasFallbackNote }> = ({ note }) => (
  <div className="canvas-fallback-mobile">
    <p className="muted">画布功能仅在桌面版支持。<br />请用电脑端编辑此便签；下方为大纲定位（只读）。</p>
    {note?.canvasOutline?.length ? (
      <ul className="canvas-fallback-outline">{note.canvasOutline.map((entry) => <li key={entry.itemId}>{entry.label}</li>)}</ul>
    ) : <p className="muted">（该画布没有章节大纲）</p>}
  </div>
);
...
const App: React.FC = () => {
  ...
  const [showPreview, setShowPreview] = useState(!isNativeMobile());
  ...
  const [showTodayPlanWindow, setShowTodayPlanWindow] = useState(false);
  const [showTimerStatsWindow, setShowTimerStatsWindow] = useState(false);
  const activeNote = notes.find((note) => note.id === activeNoteId);
  const isCanvas = activeNote?.noteType === 'canvas';

  // 移动端覆盖层：桌面多窗口语义 → 站内事件驱动
  useEffect(() => {
    const openQuickNote = () => setShowQuickNote(true);
    const closeQuickNote = () => setShowQuickNote(false);
    const openPlan = () => setShowTodayPlanWindow(true);
    const closePlan = () => setShowTodayPlanWindow(false);
    const openStats = () => setShowTimerStatsWindow(true);
    const closeStats = () => setShowTimerStatsWindow(false);
    window.addEventListener(EV_QUICK_NOTE_TOGGLE, openQuickNote);
    window.addEventListener(EV_QUICK_NOTE_CLOSE, closeQuickNote);
    window.addEventListener(EV_TODAY_PLAN_TOGGLE, openPlan);
    window.addEventListener(EV_TODAY_PLAN_CLOSE, closePlan);
    window.addEventListener(EV_STATS_TOGGLE, openStats);
    window.addEventListener(EV_STATS_CLOSE, closeStats);
    return () => {
      window.removeEventListener(EV_QUICK_NOTE_TOGGLE, openQuickNote);
      window.removeEventListener(EV_QUICK_NOTE_CLOSE, closeQuickNote);
      window.removeEventListener(EV_TODAY_PLAN_TOGGLE, openPlan);
      window.removeEventListener(EV_TODAY_PLAN_CLOSE, closePlan);
      window.removeEventListener(EV_STATS_TOGGLE, openStats);
      window.removeEventListener(EV_STATS_CLOSE, closeStats);
    };
  }, []);

  // 系统返回键：优先关闭覆盖层，其次回退页面历史
  useEffect(() => {
    const onPanelBack = (event: Event) => {
      if (showQuickNote || showTodayPlanWindow || showTimerStatsWindow) {
        event.preventDefault();
        setShowQuickNote(false);
        setShowTodayPlanWindow(false);
        setShowTimerStatsWindow(false);
      }
    };
    window.addEventListener(EV_PANEL_BACK, onPanelBack);
    return () => window.removeEventListener(EV_PANEL_BACK, onPanelBack);
  }, [showQuickNote, showTodayPlanWindow, showTimerStatsWindow]);
...
5. 每周至少导出一次综合备份。` });
  ...
  useEffect(() => {
    ...
    getBridge().kv.updateTheme(settings.theme);
  }, [settings.theme]);

  useEffect(() => {
    getBridge().kv.getSettings().then((saved) => {
  ...
  // 事件总线监听（原菜单/跨窗口事件，移动端收敛为站内事件）
  useEffect(() => {
    const bridge = getBridge();
    bridge.events.onNewNote(() => {
      createNote();
    });
    bridge.events.onExportData(async () => {
      const data = await bridge.exporter.exportData();
      if (!data) return;
    ...
    bridge.events.onImportData(() => {
      const input = document.createElement('input'); input.type = 'file'; input.accept = '.json';
      input.onchange = async () => {
        const file = input.files?.[0];
        if (!file) return;
        const result = await bridge.exporter.importData(await file.text());
        if (result?.success) await loadNotes();
        else alert('导入失败：备份文件格式无效');
      };
      input.click();
    });
    // 监听选中便签事件（来自 QuickNote 覆盖层）
    bridge.events.onSelectNote((noteId: string) => {
      selectNote(noteId);
    });
    ...
    <div className={`app-shell flex flex-col h-screen overflow-hidden bg-white dark:bg-gray-950 ${showWorkbench ? 'workbench-first' : ''} ${settings.wallpaper ? 'app-with-wallpaper' : ''}`} style={settings.wallpaper ? { backgroundImage: `url(${settings.wallpaper})`, backgroundSize: 'cover', backgroundPosition: 'center' } : undefined}>
      ...
      <div className="app-titlebar flex items-center h-9 px-2 bg-white dark:bg-gray-900 border-b border-slate-200 dark:border-gray-800 select-none flex-shrink-0"
        ...
        <div className="app-title-actions flex items-center gap-1 flex-shrink-0" style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
          ...
          {!showWorkbench && <button onClick={() => getBridge().window.toggleQuickNote()}
          ...
          {/* 窗口控制按钮（桌面语义；移动端为 no-op，Phase 4 将按平台隐藏） */}
          <div className="app-titlebar-controls flex items-center ml-1 pl-1 border-l border-gray-200 dark:border-gray-700">
            <button onClick={() => getBridge().window.winMinimize()}
            ...
            <button onClick={() => getBridge().window.winMaximize()}
              className="w-8 h-7 flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-400 transition-colors rounded">
            ...
            <button onClick={() => getBridge().window.winClose()}
      ...
      <div className="app-main-area flex-1 flex min-h-0">
        ...
        <div className="app-workspace flex-1 flex flex-col min-w-0">
          ...
          <div className="app-content-area flex-1 flex min-h-0 relative">
            {showWorkbench ? <StudyWorkbench /> : showKanban ? <KanbanBoard /> : showTodayPlan && !focusMode ? <TodayPlan /> : isCanvas ? (isNativeMobile() ? <MobileCanvasFallback note={activeNote!} /> : <CanvasBoard note={activeNote!} />) : isNativeMobile() ? (showPreview && !focusMode ? <Preview onClose={() => setShowPreview(false)} /> : <Editor />) : <><Editor />{(showPreview && !focusMode) && <Preview onClose={() => setShowPreview(false)} />}</>}
            {/* 专注模式退出提示 */}
            {focusMode && (
      ...
      {showCommandPalette && <CommandPalette notes={notes} onClose={() => setShowCommandPalette(false)} onOpenNote={(id) => { selectNote(id); setShowTodayPlan(false); setShowKanban(false); setShowWorkbench(true); }} onNewNote={createNote} onNewCanvas={isNativeMobile() ? undefined : createCanvas} onTogglePreview={() => setShowPreview((value) => !value)} onOpenAssets={() => setShowAssets(true)} onOpenAppearance={() => setShowAppearance(true)} />}
      ...
      {/* 移动端覆盖层：快速笔记 / 今日计划窗口 / 任务统计窗口 */}
      {showQuickNote && <div className="fixed inset-0 z-[80] bg-white dark:bg-gray-950"><QuickNote /></div>}
      {showTodayPlanWindow && <div className="fixed inset-0 z-[80] bg-white dark:bg-gray-950"><TodayPlanWindow /></div>}
      {showTimerStatsWindow && <div className="fixed inset-0 z-[80] bg-white dark:bg-gray-950"><TimerStatsWindow /></div>}
    </div>
  );
};
