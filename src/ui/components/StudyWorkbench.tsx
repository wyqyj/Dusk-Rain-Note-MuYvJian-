...
import { generateId, normalizeMathMarkdown, renderMarkdown } from '../../core/utils/markdown';
import { useSettingsStore } from '../store/settingsStore';
import type { Book, BookSort, FocusRecord, Mastery, Question, QuestionBook, Subject, Task, TaskBucket, Workspace } from '../../core/domain/types';
import { parseMarkdownTasks } from '../../core/utils/planTasks';
import { renderQuestionBookMarkdown, sanitizeExportFileName, selectQuestionExportItems, type QuestionExportScope } from '../../core/utils/questionBookExport';
import { getBridge } from '../../platform/container';
import { isNativeMobile } from '../utils/platform';

/** 工作台导航（移动端移除画布，功能不支持） */
const ALL_NAV = [['overview', '总览'], ['plan', '规划'], ['focus', '专注'], ['books', '书架'], ['questions', '题册'], ['notes', '笔记'], ['canvas', '画布'], ['settings', '设置']] as const;
...
export const StudyWorkbench: React.FC = () => {
  ...
  useEffect(() => { (async () => { const content = await getBridge().workspace.getWorkspaceState(); try { const restored = normalizeWorkspace(content ? JSON.parse(content) : null); const yesterday = dateOffset(-1); restored.tasks = restored.tasks.map((task) => !task.completed && task.bucket === 'daily' && task.date === yesterday ? { ...task, date: today(), sourceDate: yesterday } : task); setWorkspace(restored); } catch { setWorkspace(initialWorkspace()); } setLoaded(true); })(); }, []);
  useEffect(() => { if (!loaded) return; const timer = window.setTimeout(() => getBridge().workspace.saveWorkspaceState(JSON.stringify(workspace, null, 2)), 350); return () => window.clearTimeout(timer); }, [workspace, loaded]);
  ...
  useEffect(() => {
    ...
    workspace.books.filter((book) => book.sourcePath && !book.coverPath && !attemptedBookCoverGeneration.current.has(book.id)).forEach((book) => {
      ...
      getBridge().workspace.generateBookCover(book.sourcePath!).then((result) => {
  ...
  useEffect(() => {
    ...
    if (!skillInfo) getBridge().workspace.getQuestionBookSkill().then((result) => result?.success ? setSkillInfo(result) : setSkillInfo({ error: `无法读取题册整理技能：${result?.error || '未知错误'}` }));
    if (!planSkillInfo) getBridge().workspace.getPlanImportSkill().then((result) => result?.success ? setPlanSkillInfo(result) : setPlanSkillInfo({ error: `无法读取计划整理技能：${result?.error || '未知错误'}` }));
  ...
  const openBookFile = (book: Book) => { if (!book.sourcePath) return; getBridge().workspace.openWorkspacePath(book.sourcePath); updateBook(book.id, { lastOpenedAt: Date.now() }); };
  ...
  const importBook = async () => { const result = await getBridge().workspace.chooseBook(); if (result?.path) { addBook(result.path, result.name?.replace(/\.[^.]+$/, ''), result.originalPath, result.coverPath); if (result.coverError) setNotice(`已导入「${result.name?.replace(/\.[^.]+$/, '') || '未命名书籍'}」，但暂时无法生成首页封面：${result.coverError}`); } };
  ...
  const importQuestionBook = async () => {
    try {
      const result = await getBridge().workspace.chooseQuestionBook();
  ...
  const refreshActiveBook = async () => {
    ...
    try {
      ...
      for (const sourcePath of sourcePaths) {
        const result = await getBridge().workspace.readQuestionBook(sourcePath);
  ...
  const copyAgentPrompt = () => { if (!skillInfo?.prompt) return; navigator.clipboard.writeText(skillInfo.prompt).then(() => setNotice('已复制 Agent 提示词。')).catch(() => alert('复制失败，请重试。')); };
  const copyPlanAgentPrompt = () => { if (!planSkillInfo?.prompt) return; navigator.clipboard.writeText(planSkillInfo.prompt).then(() => setNotice('已复制计划整理 Agent 提示词。')).catch(() => alert('复制失败，请重试。')); };
  ...
  const exportQuestionBookPdf = async () => {
    ...
    try {
      ...
      const result = await getBridge().exporter.exportPdf(title, renderQuestionBookMarkdown(activeQuestionBook, selected, title));
  ...
  const nav: ReadonlyArray<readonly [string, string]> = isNativeMobile() ? ALL_NAV.filter((entry) => entry[0] !== 'canvas') : ALL_NAV;
  ...
  return <div className={`study-shell ${settings.wallpaper ? 'has-workspace-wallpaper' : ''}`} style={settings.wallpaper ? { backgroundImage: `linear-gradient(rgba(247,250,255,.56), rgba(247,250,255,.56)), url("${settings.wallpaper}")`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat' } : undefined}>
    <aside className="study-nav"><div className="study-brand"><span>暮雨笺</span><small>学习工作台</small></div>{nav.map(([id, label]) => <button key={id} className={view === id ? 'active' : ''} onClick={() => setView(id)}>{label}</button>)}<div className="study-nav-foot">v3.0.4 · 本地优先</div></aside>
    <main className="study-main">
      <header className="study-header"><div><h1>{nav.find(([id]) => id === view)?.[1]}</h1><p>{view === 'overview' ? '考研冲刺工作台' : '所有内容仅保存在本机工作台目录'}</p></div><div className="study-header-actions">{isNativeMobile() ? null : <><button onClick={() => getBridge().workspace.backupWorkspace().then((result) => setNotice(result?.success ? `已导出综合备份：${result.path}` : '已取消备份'))}>导出综合备份</button><button onClick={() => { if (confirm('恢复备份会覆盖同名工作台文件，继续？')) getBridge().workspace.restoreWorkspace().then((result) => { if (result?.success) window.location.reload(); else if (result?.error !== 'cancelled') alert(`恢复失败：${result?.error}`); }); }}>导入恢复</button></>}</div></header>
...
const WorkspaceRoot: React.FC<{ onMigrated: (message: string) => void }> = ({ onMigrated }) => {
  useEffect(() => { getBridge().workspace.getWorkspaceRoot().then((root) => { const target = document.getElementById('workspace-root'); if (target) target.textContent = root; }); }, []);
  const changeRoot = async () => { const picked = await getBridge().workspace.chooseWorkspaceRoot(); if (!picked?.path) return; const result = await getBridge().workspace.migrateWorkspace(picked.path); if (result?.success) { const target = document.getElementById('workspace-root'); if (target) target.textContent = result.destination || picked.path; onMigrated(`迁移完成，已校验 ${result.files || 0} 个文件。旧目录仍保留。`); } else alert(`迁移失败：${result?.error}`); };
