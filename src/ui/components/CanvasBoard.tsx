...
import { generateId, renderMarkdown } from '../../core/utils/markdown';
import { isCanvasBackgroundWheelTarget } from '../../core/utils/canvasWheel';
...
import { pinchTo, zoomAround } from '../../core/utils/canvasCamera';
...
export const CanvasBoard: React.FC<{ note: Note }> = ({ note }) => {
  ...
  // 移动端双指捏合手势状态：touch 标识符 → 位置；pinch 记录起始距离与中点
  const touchStateRef = useRef<{
    map: Map<number, { x: number; y: number }>;
    pinch: { d0: number; mx: number; my: number } | null;
  }>({ map: new Map(), pinch: null });
  const cameraRef = useRef<Camera>({ x: 120, y: 96, scale: 1 });
  ...
  const [message, setMessage] = useState('滚轮或双指缩放，拖拽空白处平移');
  ...
  const handleWheel = (event: React.WheelEvent<HTMLDivElement>) => {
    ...
    setCamera(zoomAround(camera, pointX, pointY, nextScale));
  };

  // ---- 移动端触摸手势：双指捏合缩放（围绕两指中点，跟随中点位移平移） ----
  const handleTouchStart = (event: React.TouchEvent<HTMLDivElement>) => {
    const state = touchStateRef.current;
    for (const touch of Array.from(event.touches)) state.map.set(touch.identifier, { x: touch.clientX, y: touch.clientY });
    if (event.touches.length === 2) {
      ...
      interactionRef.current = null; // 中断元素拖拽，交由捏合手势
    }
  };
  const handleTouchEnd = (event: React.TouchEvent<HTMLDivElement>) => {
    const state = touchStateRef.current;
    for (const touch of Array.from(event.changedTouches)) state.map.delete(touch.identifier);
  ...
  useEffect(() => {
    ...
    const onTouchMove = (event: TouchEvent) => {
      ...
      event.preventDefault(); // 阻止系统双指手势（需 passive: false）
      const rect = viewport.getBoundingClientRect();
      const [a, b] = Array.from(event.touches);
      const distance = Math.max(1, Math.hypot(a.clientX - b.clientX, a.clientY - b.clientY));
      const midX = (a.clientX + b.clientX) / 2 - rect.left;
      const midY = (a.clientY + b.clientY) / 2 - rect.top;
      const { d0, mx, my } = state.pinch;
      const cam = cameraRef.current;
      const nextScale = clampScale(cam.scale * (distance / d0));
      setCamera(pinchTo(cam, mx, my, midX, midY, nextScale));
    };
  ...
  return <div className="canvas-shell">
    <header className="canvas-header">
      ...
      <div ref={stageRef} className="canvas-viewport" onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={finishInteraction} onPointerCancel={finishInteraction} onWheel={handleWheel} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} onTouchCancel={handleTouchEnd} onContextMenu={(event) => handleContextMenu(event)} onDragOver={(event) => event.preventDefault()} onDrop={handleDrop} onPaste={handlePaste}>
