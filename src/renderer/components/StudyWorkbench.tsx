...
export const StudyWorkbench: React.FC = () => {
  ...
  const [view, setView] = useState<'overview' | 'plan' | 'focus' | 'books' | 'questions' | 'notes' | 'canvas' | 'ai' | 'agent' | 'settings'>('overview');
  ...
  // 其他页（如 AI 助手）点击「接口设置」时跳到 Agent 页并打开其 GUI 设置
  useEffect(() => {
    const goAgentSettings = () => setView('agent');
    window.addEventListener('muyujian:open-agent-settings', goAgentSettings);
    return () => window.removeEventListener('muyujian:open-agent-settings', goAgentSettings);
  }, []);

  const nav = [['overview', '总览'], ['plan', '规划'], ['focus', '专注'], ['books', '书架'], ['questions', '题册'], ['notes', '笔记'], ['canvas', '画布'], ['ai', 'AI 助手'], ['agent', 'Agent'], ['settings', '设置']] as const;
